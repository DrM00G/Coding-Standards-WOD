//I think that it's too common of a char and maybe it would also mess up for loops
...
function hasWord(lyrics: string[]): boolean {
  const has = lyrics.some((line) => line.toLowerCase().includes("hello"));
  return has;
}

function numWord(lyric: string[]): number {
  const count = lyric.filter((line) => line.toLowerCase().includes("hello")).length;
